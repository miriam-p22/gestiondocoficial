...
const obtenerLimiteFinDia = (valor) => {
...
const diasHastaLimite = (fechaLimite) => {
  const limite = obtenerLimiteFinDia(fechaLimite);
...
export const useDashboardData = ({
  ...
  const [organigramasRevision, setOrganigramasRevision] = useState([]);
  ...
  const cargarDashboard = useCallback(async () => {
    ...
    try {
      ...
      const [dataDispersiones, dataUsuarios, dataVigente, dataRevision] =
        await Promise.all(promesas);

      setDispersiones(Array.isArray(dataDispersiones) ? dataDispersiones : []);
      setUsuarios(Array.isArray(dataUsuarios) ? dataUsuarios : []);

      setOrganigramaVigente(
        dataVigente && !Array.isArray(dataVigente)
          ? dataVigente
          : Array.isArray(dataVigente) && dataVigente.length > 0
            ? dataVigente[0]
            : null,
      );

      setOrganigramasRevision(Array.isArray(dataRevision) ? dataRevision : []);
    } catch (err) {
      setError(
        err?.message || "No fue posible cargar la información del Dashboard.",
      );
    } finally {
      setLoading(false);
    }
    ...
    dispersiones.forEach((dispersion) => {
      ...
      destinos.forEach((destino) => {
        filas.push({
          ...
          nombre_area: destino.area?.nombre_area || "Sin área",
          nombre_archivo: dispersion.nombre_archivo || "Documento",
          estado: normalizarEstado(destino.estado_documento),
          fecha_limite: destino.fecha_limite,
          fecha_recepcion_area: destino.fecha_recepcion_area,
          fecha_inicio_proceso: destino.fecha_inicio_proceso,
          fecha_atencion: destino.fecha_atencion,
  ...
  const metricas = useMemo(() => {
    ...
    documentos.forEach((documento) => {
      ...
      const limite = obtenerLimiteFinDia(documento.fecha_limite);
      const atencion = fechaValida(documento.fecha_atencion);
      const activo = !["atendido", "devuelto"].includes(estado);
      ...
      if (estado === "atendido") {
        ...
        if (limite && atencion) {
          if (atencion.getTime() <= limite.getTime()) {
            atendidosATiempo += 1;
          } else {
            atendidosFueraPlazo += 1;
          }
        }
      ...
      const dias = diasHastaLimite(documento.fecha_limite);
    ...
    const baseCumplimiento = atendidosATiempo + atendidosFueraPlazo;

    const cumplimiento =
      baseCumplimiento > 0
        ? Math.round((atendidosATiempo / baseCumplimiento) * 100)
        : 0;
  ...
  const resumenAreas = useMemo(() => {
    ...
    documentos.forEach((documento) => {
      const id = Number(documento.id_area);

      if (!mapa.has(id)) {
        mapa.set(id, {
          id,
          area: documento.nombre_area,
          total: 0,
          atendidos: 0,
          aTiempo: 0,
          fueraPlazo: 0,
      ...
      if (
        limite &&
        new Date().getTime() > limite.getTime() &&
        !["atendido", "devuelto"].includes(documento.estado)
      ) {
        fila.vencidos += 1;
      }
    });
    ...
    return Array.from(mapa.values())
      .map((item) => {
        const base = item.aTiempo + item.fueraPlazo;

        return {
          ...item,

          cumplimiento: base > 0 ? Math.round((item.aTiempo / base) * 100) : 0,
        };
      })
  ...
  const documentosRecientes = useMemo(
    () =>
      [...documentos]
        .sort((a, b) => {
          const fechaA = fechaValida(
            a.fecha_recepcion_area ||
              a.fecha_inicio_proceso ||
              a.fecha_atencion,
          );

          const fechaB = fechaValida(
  ...
  //USUARIOS
  const metricasUsuarios = useMemo(() => {
