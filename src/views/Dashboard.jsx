...
const Dashboard = () => {
  ...
  // NAVEGACION
  const irADocumentos = (filtro = "") => {
    if (filtro) {
      navigate(`/Documentos?filtro=${encodeURIComponent(filtro)}`);
  ...
  const cards = useMemo(() => {
    if (esPresidencia) {
      return [
        ...
        {
          title: "Atendidos a tiempo",
          value: metricas.atendidosATiempo,
          subtitle: "Dentro del plazo",
          icon: <FiCheckCircle />,
          tone: "success",
          onClick: () => irADocumentos("atendidos"),
        },
        {
          ...
          icon: <FiClock />,
          tone: "warning",
          onClick: () => irADocumentos("por-vencer"),
        },
        {
          title: "Vencidos",
          value: metricas.vencidos,
          subtitle: "Aún sin atención",
          icon: <FiAlertTriangle />,
          tone: "danger",
          onClick: () => irADocumentos("vencidos"),
        },
      ];
    }
    ...
    if (esAdministrador) {
      return [
        ...
        {
          title: "Cumplimiento",
          value: `${metricas.cumplimiento}%`,
          subtitle: "Atención dentro de plazo",
          icon: <FiCheckCircle />,
          tone: "success",
          onClick: () => irADocumentos(),
        },
    ...
    if (esRecursosHumanos) {
      return [
        ...
        {
          title: "Atendidos",
          value: metricas.atendidos,
          subtitle: "Documentos atendidos",
          icon: <FiCheckCircle />,
          tone: "success",
          onClick: () => irADocumentos("atendidos"),
        },
        {
          title: "Usuarios",
          value: metricasUsuarios.total,
          subtitle: `${metricasUsuarios.activos} activos`,
          icon: <FiUsers />,
          tone: "neutral",
          onClick: () => irADocumentos("recibidos"),
        },
      ];
    }
  ...
  const alertas = useMemo(() => {
    ...
    if (metricas.vencidos > 0) {
      lista.push({
        ...
        text: `${metricas.vencidos} documento(s) requieren atención inmediata.`,
      });
    }
    ...
    if (metricas.porVencer > 0) {
      lista.push({
        type: "warning",
        title: "Próximos a vencer",
        text: `${metricas.porVencer} documento(s) vencen dentro de los próximos 3 días.`,
      });
    }
    ...
    if (lista.length === 0) {
      lista.push({
        ...
        text: "No hay incidencias urgentes en este momento.",
      });
    }
  ...
  const tabla = useMemo(() => {
    if (esAdministrador || esPresidencia) {
      return {
        ...
        subtitle: "Comparativa de atención documental.",
        ...
        columns: [
          ...
          {
            key: "cumplimientoTexto",
            label: "Cumplimiento",
          },
        ],

        rows: resumenAreas.map((item) => ({
          ...item,
    ...
    return {
      ...
      subtitle: "Últimos documentos asignados al área.",
      ...
      rows: documentosRecientes.map((item) => ({
        ...item,

        estadoTexto: item.estado.charAt(0).toUpperCase() + item.estado.slice(1),
  ...
  const documentosFiltrados = useMemo(() => {
    ...
    return documentosSeguimiento
      .filter((documento) => {
        if (filtroArea && Number(documento.id_area) !== Number(filtroArea))
          return false;
      ...
      .sort((a, b) => {
        ...
        return (
          (prioridad[obtenerEstadoVisual(a)] ?? 99) -
  ...
  // ACTIVIDAD
  const actividad = documentosRecientes.slice(0, 4).map((item, index) => ({
    id: `${item.id}-${index}`,

    title: item.nombre_archivo,

    text: `${item.nombre_area} — ${item.estado}`,
  ...
  if (esAdministrador) {
    ...
    accesos.push({
      ...
      title: "Configuración",
      icon: <FiSettings />,
      onClick: () => irAModulo("/config/notificacion-conexion"),
    });
  }
  ...
  if (esRecursosHumanos) {
    accesos.push({
      ...
      icon: <FiWifi />,
      onClick: () => irAModulo("/config/direcciones-ip"),
    });
  }
  ...
  return (
    <main className="content-area">
      <section className="content-section dashboard-page">
        ...
        {loading ? (
          <div className="dashboard-panel">Cargando indicadores...</div>
        ) : (
          <>
              ...
              <section className="dashboard-panel dashboard-followup-panel">
                <div className="dashboard-panel-header dashboard-followup-header">
                  <div>
                    ...
                    <p>
                      Consulta detallada del avance y documentos que requieren
                      atención en las áreas municipales.
                    </p>
                  </div>
                ...
                <div className="dashboard-followup-summary">
                  <div>
                    <span>Total</span>
                    <strong>{resumenSeguimiento.total}</strong>
                  </div>
                  <div>
                    <span>Pendientes</span>
                    <strong>{resumenSeguimiento.pendientes}</strong>
                  </div>
                  <div>
                    <span>En proceso</span>
                    <strong>{resumenSeguimiento.enProceso}</strong>
                  </div>
                  <div className="dashboard-followup-summary-danger">
                    <span>Vencidos</span>
                    <strong>{resumenSeguimiento.vencidos}</strong>
                  </div>
                  <div className="dashboard-followup-summary-success">
                    <span>Atendidos</span>
                    <strong>{resumenSeguimiento.atendidos}</strong>
                  </div>
                </div>
                      ...
                      {documentosFiltrados.length > 0 ? (
                        documentosFiltrados.map((documento) => {
                          ...
                          return (
                            <tr key={documento.id}>
                              <td>
                                <strong className="dashboard-followup-area">
                              ...
                              <td>
                                <span
                                  className={`dashboard-status-badge dashboard-status-${estado.replace(/\s+/g, "-")}`}
                                >
                              ...
                              <td>
                                {formatearFecha(documento.fecha_recepcion_area)}
                              </td>
                              <td>
                                {formatearFecha(documento.fecha_inicio_proceso)}
                              </td>
                              <td>
                                {formatearFecha(documento.fecha_atencion)}
                              </td>
                            </tr>
                          );
                        })
                        ...
                        <tr>
                          <td colSpan="7" className="dashboard-followup-empty">
                ...
                >
                  <div className="dashboard-bars">
                    {[
                      ...
                      const maximo = Math.max(metricas.total, 1);
              ...
              <aside className="dashboard-side-column">
                <section className="dashboard-panel">
                  <div className="dashboard-panel-header">
                    <div>
                      ...
                      <p>Información que requiere atención.</p>
                    </div>
                  </div>
                ...
                {organigramaVigente && (
                  <section className="dashboard-panel">
                    <div className="dashboard-panel-header">
                      <div>
                        <h3>Organigrama vigente</h3>

                        <p>Última versión autorizada.</p>
                      </div>
                    </div>

                    <div className="dashboard-organigrama-summary">
                      <strong>{organigramaVigente.titulo}</strong>

                      <span>Versión {organigramaVigente.version}</span>
                ...
                <DashboardQuickAccess items={accesos} />
