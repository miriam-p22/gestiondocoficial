...
const AreasPage = () => {
  ...
  const puedeGestionar = !loadingPermisos && tienePrivilegio("Gestionar Áreas");
  ...
  const [usoArea, setUsoArea] = useState(null);

  const columns = puedeGestionar ? ["ID", "Área", "Acciones"] : ["ID", "Área"];
  ...
  const handleCrear = async () => {
    ...
    try {
      ...
      setMensaje("Área agregada correctamente.");
    } catch (err) {
      setErrorFormulario(err?.message || "No fue posible crear el área.");
    }
  };
  ...
  const handleEditar = async () => {
    ...
    try {
      ...
      await actualizarArea(areaSeleccionada.id, nombre);
      ...
      setErrorFormulario(err?.message || "No fue posible actualizar el área.");
    }
  };

  const abrirEliminar = async (area) => {
    if (!puedeGestionar) {
      return;
    }
    ...
    try {
      const uso = await obtenerUsoArea(area.id);
  ...
  const handleEliminar = async () => {
    ...
    try {
      await eliminarArea(areaSeleccionada.id);
      ...
      setErrorFormulario(err?.message || "No fue posible eliminar el área.");
    }
  };

  const tableData = areasFiltradas.map((area) => ({
    _id: area.id,
    ...
    <main className="content-area">
      <section className="content-section areas-page">
        <div className="areas-header">
          <div>
            <h2 className="card-title">Gestión de Áreas</h2>
        ...
        <div className="areas-permission-info">
          ...
          <div>
            <strong>
              {puedeGestionar ? "Gestión habilitada" : "Consulta de áreas"}
            </strong>
        ...
        <Card className="areas-table-card">
          {loading || loadingPermisos ? (
            <div className="areas-loading">Cargando áreas...</div>
          ) : areasFiltradas.length === 0 ? (
            <div className="areas-empty">No se encontraron áreas.</div>
          ) : (
            <TablaReutilizable
              columns={columns}
              data={tableData}
            ...
            {usoArea && (
              <div className="area-uso-info">
                <div className="area-uso-grid">
                  <div>
                    <span>Usuarios</span>
                    <strong>{usoArea.usuarios}</strong>
                  </div>

                  <div>
                    <span>Documentos</span>
                    <strong>{usoArea.documentos}</strong>
                  </div>

                  <div>
                    <span>Organigramas</span>
                    <strong>{usoArea.niveles}</strong>
                  </div>

                  <div>
                    <span>Direcciones IP</span>
                    <strong>{usoArea.ips}</strong>
                  </div>
                </div>

                {usoArea.en_uso && (
                  <div className="area-en-uso-warning">
                    Esta área está siendo utilizada y no podrá eliminarse hasta
                    reasignar sus relaciones.
                  </div>
                )}
              </div>
            )}
