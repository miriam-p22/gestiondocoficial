...
const prepararDatosApi = (formulario, isEdit = false) => {
  const datos = {
    nombre_completo: formulario.nombre_completo.trim(),

    numero_trab:
      formulario.numero_trab === "" ? null : Number(formulario.numero_trab),
    ...
    id_area: Number(formulario.id_area),

    status: formulario.status === true || formulario.status === "true",
  };

  if (!isEdit || String(formulario.contrasenia || "").trim() !== "") {
    datos.contrasenia = String(formulario.contrasenia);
  }
...
const Usuarios = () => {
  ...
  const puedeGestionarRoles =
    !loadingPermisos && tienePrivilegio("Gestionar Roles y Privilegios");
  ...
  // MODAL PRIVILEGIOS
  const [isModalPrivilegiosOpen, setIsModalPrivilegiosOpen] = useState(false);

  const [privilegiosSeleccionados, setPrivilegiosSeleccionados] = useState([]);

  const [usuarioPrivilegios, setUsuarioPrivilegios] = useState(null);

  const [errorModalPrivilegios, setErrorModalPrivilegios] = useState("");

  // FORMULARIO
  const [currentUser, setCurrentUser] = useState(formularioInicial);
  ...
  const abrirAgregar = () => {
    ...
    setCurrentUser({
      ...
      id_area: areas.length > 0 ? areas[0].id : "",
    });
  ...
  const cerrarAgregar = () => {
    ...
    setCurrentUser(formularioInicial);
  ...
  const abrirEditar = (usuario) => {
    ...
    setCurrentUser(normalizarFormulario(usuario));
  ...
  //VALIDACIÓN
  const validarFormulario = (isEdit = false) => {
    if (!String(currentUser.nombre_completo || "").trim()) {
      throw new Error("El nombre completo es obligatorio.");
    }

    if (!String(currentUser.nombre_usuario || "").trim()) {
      throw new Error("El nombre de usuario es obligatorio.");
    }

    if (!isEdit && !String(currentUser.contrasenia || "").trim()) {
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      const datos = prepararDatosApi(currentUser, false);

      const nuevoUsuario = await crearUsuario(datos);

      setSelectedUserId(nuevoUsuario.id);
      ...
      setErrorFormulario(
        errorCreacion?.message || "No fue posible crear el usuario.",
      );
    }
  };
  ...
  const handleEditUser = async () => {
    ...
    try {
      ...
      const datos = prepararDatosApi(currentUser, true);

      await actualizarUsuario(currentUser.id, datos);
      ...
      setErrorEdicion(
        errorActualizacion?.message || "No fue posible actualizar el usuario.",
      );
    }
  };

  // MENÚ DE ESTADO
  const toggleMenuEstado = (event, usuario) => {
    event.stopPropagation();

    if (estadoMenu?.id === usuario.id) {
      setEstadoMenu(null);
      return;
    }
  ...
  const handleStatusChange = async (usuario, nuevoEstado) => {
    ...
    try {
      ...
      setMensaje(errorEstado?.message || "No fue posible cambiar el estado.");
    }
  };
  ...
  const handleDeleteUser = async (usuario) => {
    ...
    try {
      ...
      setMensaje(
        errorEliminacion?.message || "No fue posible eliminar el usuario.",
      );
    }
  };
  ...
  const abrirPrivilegios = async () => {
    ...
    if (!puedeGestionarRoles) {
      setMensaje("No tiene permiso para gestionar roles y privilegios.");
    ...
    if (!usuarioSeleccionado.id_rol) {
      setMensaje("El usuario seleccionado no tiene un rol válido.");

      return;
    }

    setUsuarioPrivilegios(usuarioSeleccionado);
    ...
    try {
      const resultado = await cargarPrivilegiosDelRol(
        usuarioSeleccionado.id_rol,
      );

      const idsAsignados = resultado.permisos.map(
        (permiso) => permiso.id_privilegio,
      );

      setPrivilegiosSeleccionados(idsAsignados);
    } catch (errorCarga) {
      setErrorModalPrivilegios(
        errorCarga?.message || "No fue posible cargar los privilegios.",
      );
    }
  };
  ...
  const togglePrivilegio = (idPrivilegio) => {
    setErrorModalPrivilegios("");

    setPrivilegiosSeleccionados((actuales) => {
      if (actuales.includes(idPrivilegio)) {
        return actuales.filter((id) => id !== idPrivilegio);
      }

      return [...actuales, idPrivilegio];
    });
  };

  const handleGuardarPrivilegios = async () => {
    if (!puedeGestionarRoles) {
      setErrorModalPrivilegios(
    ...
    if (!usuarioPrivilegios) {
      setErrorModalPrivilegios("No existe un usuario seleccionado.");
    ...
    try {
      ...
      const nombreRol = usuarioPrivilegios.rol?.nombre_rol || "seleccionado";
      ...
      setErrorModalPrivilegios(
        errorGuardado?.message || "No fue posible guardar los privilegios.",
      );
    }
  };

  const tableData = usuariosFiltrados.map((usuario) => {
    const estatus = usuario.status ? "Activo" : "Inactivo";
    ...
    return {
      ...
      Usuario: {
        main: (
          <div className="usuario-identidad">
            ...
            >
              @{usuario.nombre_usuario || "sin_usuario"}
            </span>

            <span
              className="usuario-identidad-correo"
              title={usuario.correo_electronico || "Sin correo electrónico"}
            >
      ...
      Adscripción: {
        main: (
          <span
            ...
            title={usuario.area?.nombre_area || "Sin área"}
          >
            {usuario.area?.nombre_area || "Sin área"}
          </span>
        ),
      },
      ...
      Rol: {
        main: (
          <span
            className="usuario-texto-ajustable"
            title={usuario.rol?.nombre_rol || "Sin rol"}
          >
      ...
      Estatus: {
        main: <EtiquetaEstado estatus={estatus} />,
      },
        ...
        main: (
          <div className="usuarios-actions-cell">
            ...
            <button
              ...
              aria-expanded={estadoMenu?.id === usuario.id}
              onClick={(event) => toggleMenuEstado(event, usuario)}
            >
              <FiRefreshCw />
            </button>
    ...
    <div className="main-content-wrapper">
          ...
          <div className="table-container">
            ...
            <div className="management-buttons-container">
              <BotonReutilizable
                ...
                disabled={saving || roles.length === 0 || areas.length === 0}
              >
                Agregar Usuario
              </BotonReutilizable>
      ...
      {puedeGestionarRoles && (
        <ModalReutilizable
          ...
          isOpen={isModalPrivilegiosOpen}
          onClose={cerrarPrivilegios}
          onAccept={handleGuardarPrivilegios}
          acceptButtonText="Guardar"
          loading={savingPrivilegios}
          errorMessage={errorModalPrivilegios || errorPrivilegios}
        >
          {usuarioPrivilegios && (
            <>
              <div className="privilegios-info">
                <div>
                  <strong>Usuario:</strong> {usuarioPrivilegios.nombre_completo}
                </div>

                <div>
                  <strong>Rol:</strong>{" "}
                  {usuarioPrivilegios.rol?.nombre_rol || "Sin rol"}
                </div>
              </div>

              <div className="privilegios-advertencia">
                Los privilegios se asignan al rol. Los cambios afectarán a todos
                los usuarios que tengan el rol{" "}
                <strong>{usuarioPrivilegios.rol?.nombre_rol || ""}</strong>.
              </div>
            </>
          )}
          ...
          <FormularioPrivilegios
            ...
            seleccionados={privilegiosSeleccionados}
            onChange={togglePrivilegio}
            loading={loadingPrivilegios}
          />
        </ModalReutilizable>
      )}
        ...
        createPortal(
          <>
            ...
            <div
              ...
              style={{
                top: `${estadoMenu.top}px`,
                left: `${estadoMenu.left}px`,
              }}
            ...
            >
              <button
                ...
                className={`usuario-status-option usuario-status-option-active ${
                  estadoMenu.usuario.status ? "is-current" : ""
                }`}
                disabled={estadoMenu.usuario.status}
                onClick={() => handleStatusChange(estadoMenu.usuario, true)}
              >
              ...
              <button
                ...
                disabled={!estadoMenu.usuario.status}
                onClick={() => handleStatusChange(estadoMenu.usuario, false)}
              >
