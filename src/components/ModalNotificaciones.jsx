...
const ModalNotificaciones = ({
  ...
  const unreadCount = notifications.filter(
    (notification) => notification?.leida !== true,
  ).length;

  const handleNotificationClick = async (notification) => {
    if (!notification || notification.leida === true || !onMarkAsRead) {
      return;
    }
    ...
    try {
      ...
      console.error("[ModalNotificaciones] Error al marcar como leída:", error);
    }
  };
  ...
  const handleMarkAll = async () => {
    ...
    try {
      ...
      console.error("[ModalNotificaciones] Error al marcar todas:", error);
    }
  };
  ...
  return (
    ...
    >
      <div className="notif-modal-header">
        ...
        >
          {saving ? "Procesando..." : "Marcar todas como leídas"}
        </button>
      </div>
      ...
      <div className="notif-list">
        {loading ? (
          <div className="notif-empty">Cargando notificaciones...</div>
          ...
          notifications.map((notification) => {
            ...
            return (
              <button
                key={notification.id}
                type="button"
                className={`notif-item ${leida ? "read" : "unread"}`}
                onClick={() => handleNotificationClick(notification)}
                disabled={saving}
                title={leida ? "Notificación leída" : "Marcar como leída"}
              >
                <div className="notif-item-left">
                  {!leida && <span className="notif-dot" />}
