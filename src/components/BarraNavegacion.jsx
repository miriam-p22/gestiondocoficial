...
const NotificationItem = ({ notification, onClick }) => {
  const leida = notification?.leida === true;

  return (
    <div
      className={`notification-item ${leida ? "read" : "unread"}`}
    ...
    >
      <div className="notification-item-left">
        {!leida && <span className="notif-dot-mini" />}
        ...
        <div>
          ...
          <p className="notification-message">
            {notification?.mensaje || "Notificación del sistema"}
          </p>

          <span className="notification-time">
            {obtenerTiempoRelativo(notification?.fecha_creacion)}
          </span>
        </div>
      </div>

      <span className={`notif-pill ${leida ? "pill-read" : "pill-new"}`}>
        {leida ? "Leída" : "Nueva"}
      </span>
    </div>
  );
...
function Navbar({
  ...
  const totalNoLeidas = Number.isInteger(unreadCount)
    ? unreadCount
    : notifications.filter((notification) => notification?.leida !== true)
        .length;

  const handleNotificationToggle = async () => {
    if (!notifMenuOpen && onRefreshNotifications) {
      try {
  ...
  const handleNotificationClick = async (notification) => {
    ...
    if (notification.leida !== true && onMarkAsRead) {
      try {
    ...
    const ruta = String(notification.ruta || "").trim();
  ...
  return (
    <header className="navbar">
      ...
      <div className="center-items">
        ...
        <div className="right-item">
          <img src={IconoGrupo} alt="Rol" className="icon-control-img" />
        ...
        <div className="notification-dropdown-container">
          ...
          >
            <p className="dropdown-title">
              ...
              {totalNoLeidas > 0 && ` (${totalNoLeidas} nuevas)`}
            </p>
