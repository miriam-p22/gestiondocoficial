...
const getOne = async (idRol, idPrivilegio) => {
  const permiso = await prisma.rolPermiso.findUnique({
    where: {
      id_rol_id_privilegio: {
        id_rol: convertirId(idRol),
        id_privilegio: convertirId(idPrivilegio),
      },
    },
    include: {
      rol: true,
...
const create = async (data) => {
  ...
  return prisma.rolPermiso.create({
    data: {
      ...
      id_privilegio: convertirId(data.id_privilegio),
    },
    include: {
      rol: true,
      privilegio: true,
...
const replaceByRol = async (idRol, privilegios = []) => {
  ...
  const ids = [...new Set(privilegios.map((id) => convertirId(id)))];
  ...
  return prisma.$transaction(async (transaction) => {
    ...
    if (ids.length > 0) {
      const existentes = await transaction.privilegio.count({
        where: {
          id: {
            in: ids,
          },
      ...
      if (existentes !== ids.length) {
        throw new Error("PRIVILEGE_NOT_FOUND");
      }
    }
...
const remove = async (idRol, idPrivilegio) => {
  try {
